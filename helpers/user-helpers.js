...
var instance = new Razorpay({
  ...
  key_secret: "tH4vJwFji3WLnDms9bT9sPvL",
});
...
module.exports = {
  ...
  addToCart: (productId, userId, user) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let userCart = await client
        .db(dataBase.DBNAME)
        .collection(dataBase.CART_COLLECTION)
        .findOne({ user: userId });
      if (userCart) {
  ...
  getCartProducts: (userId) => {
    return new Promise(async (resolve, reject) => {
      let cartUser = await client
        .db(dataBase.DBNAME)
        .collection(dataBase.CART_COLLECTION)
        .findOne({ user: userId });
      if (cartUser) {
        let cartItems = await client
          .db(dataBase.DBNAME)
          .collection(dataBase.CART_COLLECTION)
          .aggregate([
            {
  ...
  verifyPayment: (detailes) => {
    return new Promise((resolve, reject) => {
      ...
      let hmac = crypto.createHmac("sha256", "tH4vJwFji3WLnDms9bT9sPvL");
  ...
  updateProfile: (userData) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let user = await client
        .db(dataBase.DBNAME)
        .collection(dataBase.USER_COLLECTION)
        .findOne({ email: userData.newEmail });
